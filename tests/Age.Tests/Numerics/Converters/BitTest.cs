...
{
    private static void AssertIt(ReadOnlySpan<byte> actual, ReadOnlySpan<byte> expected) =>
        Assert.True(actual.SequenceEqual(expected));

    private static void AssertIt(ReadOnlySpan<ushort> actual, ReadOnlySpan<ushort> expected) =>
        Assert.True(actual.SequenceEqual(expected));
