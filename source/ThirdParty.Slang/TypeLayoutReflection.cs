...
public unsafe class TypeLayoutReflection : Managed<TypeLayoutReflection>
{
    [field: AllowNull]
    ...
    [field: AllowNull]
    public VariableLayoutReflection? ContainerVarLayout => field ??= PInvoke.spReflectionTypeLayout_getContainerVarLayout(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public TypeLayoutReflection? ElementTypeLayout => field ??= PInvoke.spReflectionTypeLayout_GetElementTypeLayout(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public VariableLayoutReflection? ElementVarLayout => field ??= PInvoke.spReflectionTypeLayout_GetElementVarLayout(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public VariableLayoutReflection? ExplicitCounter => field ??= PInvoke.spReflectionTypeLayout_GetExplicitCounter(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public VariableLayoutReflection[] Fields
    {
        get
        {
            if (field == null)
            {
                field = new VariableLayoutReflection[this.FieldCount];
    ...
    [field: AllowNull]
    public TypeLayoutReflection? PendingDataTypeLayout => field ??= PInvoke.spReflectionTypeLayout_getPendingDataTypeLayout(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public VariableLayoutReflection? SpecializedTypePendingDataVarLayout => field ??= PInvoke.spReflectionTypeLayout_getSpecializedTypePendingDataVarLayout(this.Handle) is var x && x != default ? new(this.Session, x) : null;

    [field: AllowNull]
    public TypeReflection? Type => field ??= PInvoke.spReflectionTypeLayout_GetType(this.Handle) is var x && x != default ? new(this.Session, x) : null;
    ...
    internal TypeLayoutReflection(Session session, Handle<TypeLayoutReflection> handle) : base(session, handle)
    { }
    ...
    public TypeLayoutReflection GetBindingRangeLeafTypeLayout(long index) =>
        new(this.Session, PInvoke.spReflectionTypeLayout_getBindingRangeLeafTypeLayout(this.Handle, index));

    public VariableReflection GetBindingRangeLeafVariable(long index) =>
        new(this.Session, PInvoke.spReflectionTypeLayout_getBindingRangeLeafVariable(this.Handle, index));
    ...
    public VariableLayoutReflection GetFieldByIndex(uint index) =>
        new(this.Session, PInvoke.spReflectionTypeLayout_GetFieldByIndex(this.Handle, index));
    ...
    public VariableLayoutReflection GetSubObjectRangeOffset(long subObjectRangeIndex) =>
        new(this.Session, PInvoke.spReflectionTypeLayout_getSubObjectRangeOffset(this.Handle, subObjectRangeIndex));
    ...
    public TypeLayoutReflection GetSubObjectRangeTypeLayout(long index) =>
        new(this.Session, PInvoke.spReflectionTypeLayout_getSubObjectRangeTypeLayout(this.Handle, index));
