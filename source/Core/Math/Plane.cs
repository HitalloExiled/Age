...
internal record struct Plane<T> where T : notnull, INumber<T>, IRootFunctions<T>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public Vector3<T>? Intersect(Plane<T> plane1, Plane<T> plane2)
    {
        ...
        if (Functions.IsZeroApprox((RealT)(object)denom))
        {
            return null;
        }

        return ((normal1.Cross(normal2) * this.Distance) + (normal2.Cross(normal0) * plane1.Distance) + (normal0.Cross(normal1) * plane2.Distance)) / denom;
    }
}
