...
[Serializable]
[DataContract]
internal record struct Projection<T>
where T : notnull, INumber<T>, IRootFunctions<T>, ITrigonometricFunctions<T>
{
    ...
    private static T GetFovY(T fovX, T aspect) =>
        Functions.RadToDeg(T.Atan(aspect * T.Tan(Functions.DegToRad(fovX) * T.CreateChecked(0.5))) * T.CreateChecked(2.0));
    ...
    public void SetPerspective(T fovYDegrees, T aspect, T zNear, T zFar, bool flipFov)
    {
        ...
        var radians = Functions.DegToRad(fovYDegrees / T.CreateChecked(2));
