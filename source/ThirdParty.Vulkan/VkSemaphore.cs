...
public unsafe partial class VkSemaphore : DeviceResource<VkSemaphore>
{
    internal VkSemaphore(VkDevice device, in VkSemaphoreCreateInfo createInfo) : base(device)
    {
        ...
        {
            VkException.Check(PInvoke.vkCreateSemaphore(device, pCreateInfo, PointerHelper.NullIfDefault(this.Instance.Allocator, pAllocator), pHandle));
        }
    }
    ...
    protected override void OnDispose()
    {
        fixed (VkAllocationCallbacks* pAllocator = &this.Instance.Allocator)
        {
            PInvoke.vkDestroySemaphore(this.Device, this, PointerHelper.NullIfDefault(this.Instance.Allocator, pAllocator));
        }
    }
}
